...
export async function checkout(params: {
  ...
  if (params.primaryAddress) {
    ...
    if (!primaryAddr) {
      throw ApiError.badRequest("No primary address found. Please add and set a primary address first.");
    }
    shippingAddress = {
    ...
    if (!params.address) {
      throw ApiError.badRequest("Address is required when primaryAddress is false");
    }
  ...
  if (idempotencyKey) {
    ...
    if (existing.length > 0) return { orders: existing.map(toPublicOrder), totalAmount: 0 };
  }
  ...
  try {
    ...
    const totalAmount = publicOrders.reduce((sum, order) => {
    ...
    return { orders: orders.map(toPublicOrder), totalAmount };
  } finally {
    session.endSession();
  }
}